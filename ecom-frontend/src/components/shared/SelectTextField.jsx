...
const SelectTextField = ({
    ...
    // Ensure `select` always has a default value when lists load
    useEffect(() => {
        if (lists?.length && !select) {
            setSelect(lists[0]);
        }
    }, [lists, select, setSelect]);

    return (
        <Listbox value={select || lists?.[0] || { categoryName: "Select category" }} onChange={setSelect}>
                ...
                <div className="relative">
                    ...
                    >
                        <span className="block truncate ps-2">
                            {select?.categoryName || "Select category"}
                        </span>
                    </ListboxButton>
